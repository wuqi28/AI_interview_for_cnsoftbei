...
def parse_transcription_result(result: dict) -> str:
    ...
    return "".join(cw.get("w", "") for segment in lattice for rt in segment.get("json_1best", {}).get("st", {}).get("rt", []) for ws in rt.get("ws", []) for cw in ws.get("cw", []))
...
if __name__ == '__main__':
    file_path = r"./audio/voice_20250529165505.wav"
    print(video_to_text(file_path))
